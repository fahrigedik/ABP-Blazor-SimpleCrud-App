...
** Check the `ConnectionStrings` in `appsettings.json` files under the `SimpleCrudApplication.Blazor` and `SimpleCrudApplication.DbMigrator` projects and change it if you need.
**
...
* Run `SimpleCrudApplication.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `SimpleCrudApplication.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `SimpleCrudApplication.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
